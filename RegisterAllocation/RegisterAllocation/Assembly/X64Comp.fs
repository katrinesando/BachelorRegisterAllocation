...
let rec cStmt stmt (varEnv : varEnv) (funEnv : funEnv) graph : varEnv * x86 list =
    ...
    | DIf(e, stmt1, stmt2,info) ->
        ...
        | Temp(n, _) ->
            ...
            | Spill ->
                ...
                env1, eCode @ preserve tempReg info (getValOfTemp n tempReg env1
                                                   @ [Ins2("cmp", Reg tempReg, Cst 0)]) graph @ code
            | r ->
    ...
    | DWhile(e, body,info) ->
        ...
        | Temp(n,_) ->
            ...
            | Spill ->
                ...
                env, loopCode env @ code @
                                  preserve tempReg info (getValOfTemp n tempReg env
    ...
    | DExpr(e,info) -> 
        ...
        | Temp(n,_) ->
            match Map.find n graph with
            | Spill -> //TODO find alternative to evictAndRestore
                let tempReg = getTemp Dummy info graph
                let env, code = cExpr e varEnv funEnv tempReg info graph
    ...
    | Assign(acc, e) ->
        ...
        | Temp(n, _) ->
            ...
            | Spill ->
                ...
                | _ ->
                    env2, eCode @ accCode @ preserve reg liveVars [Ins2("mov", Ind tr, Reg reg)] graph
            | r ->
                let env2, eCode = cExpr e env1 funEnv reg liveVars graph
                match accCode with //If empty list, accCode register contains values
                | [] ->
    ...
    | AccDeref e ->
        ...
        |Temp(n,expr) ->
            ...
            | _ ->
                ...
                match Map.find n graph with
                | Spill ->
                    code @ getValOfTemp n reg env @ [Ins2("mov", Reg reg, Ind reg)],env,reg
             ...
             match Map.find n graph with
                | Spill -> //failwith "not implemented accIndex"
                            if tr <> reg then
                                ...
                                printfn "always here :)"
                                let tempReg = getTemp reg liveVars graph
                                let env2, idxCode = cExpr idx env1 funEnv tempReg liveVars graph
                                accCode @ [Ins2("mov", Reg reg, Ind reg)] @ idxCode @
                                (preserve tempReg liveVars [Ins2("sal", Reg tempReg, Cst 3);Ins2("sub", Reg reg, Reg tempReg)] graph),env2, reg
                | r ->
                        let env2, idxCode = cExpr idx env1 funEnv reg liveVars graph
                        accCode @ [Ins2("mov", Reg tr, Ind tr)] @ idxCode
