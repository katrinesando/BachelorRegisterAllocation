...
let allocate (kind : int -> var) (typ, x) (varEnv : varEnv) asmcode: varEnv * x86 list =
    ...
    | TypA (t, Some i) -> 
      ...
      let code = Ins("mov rax, rsp") ::
                  ...
                  Ins1("push", Reg Rax) :: asmcode
      (newEnv, code) 
    | _ -> 
      let newEnv = ((x, (kind fdepth, typ)) :: env, fdepth+1)
      let code = Ins "sub rsp, 8" ::asmcode //4 originalt
      (newEnv, code)
...
and cExpr (e : expr) (varEnv : varEnv) (funEnv : funEnv) (tr : reg64) (pres : reg64 list) asmcode : x86 list = 
    match e with
    | Access acc     ->
        cAccess acc varEnv funEnv tr pres (Ins2("mov", Reg tr, Ind tr)::asmcode) 
    | Assign(acc, e) -> 
        let tr' = getTempFor (tr :: pres) 
        in cAccess acc varEnv funEnv tr' (tr :: pres)
           @ cExpr e varEnv funEnv tr (tr' :: pres) asmcode
           @ [Ins2("mov", Ind tr', Reg tr)]
    | CstI i         ->
        Ins2("mov", Reg tr, Cst i) :: asmcode
    | Addr acc       ->
        cAccess acc varEnv funEnv tr pres asmcode
    | Prim1(ope, e1) ->
        cExpr e1 varEnv funEnv tr pres asmcode
        :: (match ope with
           | "!"      -> Ins("xor rax, rax") ::
                          Ins2("cmp", Reg tr, Reg Rax) ::
                          Ins("sete al") ::
                          Ins2("mov", Reg tr, Reg Rax) :: asmcode
           | "printi" -> Ins2("mov",Reg Rdi, Reg tr)::PRINTI :: asmcode
           | "printc" -> Ins2("mov",Reg Rdi, Reg tr)::PRINTC :: asmcode
    ...
    | Prim2(ope, e1, e2) ->
        ...
        @ let tr' = getTempFor (avoid @ pres)
          in cExpr e2 varEnv funEnv tr' (tr :: pres) asmcode
             @ match ope with
               | "+"   -> Ins2("add", Reg tr, Reg tr') :: asmcode
               | "-"   -> Ins2("sub", Reg tr, Reg tr')  :: asmcode
               ...
               | "%"   -> Ins2("mov", Reg Rax, Reg tr)
                          :: preserve Rdx (tr :: pres) ::
                            Ins("cdq")::          // Invalidates Rdx
                             Ins1("idiv", Reg tr'):: // Invalidates Rax Rdx
                             Ins2("mov", Reg tr, Reg Rdx) ::asmcode 
               | "==" | "!=" | "<" | ">=" | ">" | "<="
    ...
    | Andalso(e1, e2) ->
        ...
        :: Ins2("cmp", Reg tr, Cst 0) ::
           Jump("jz", labend) 
        :: cExpr e2 varEnv funEnv tr pres asmcode
        :: Label labend :: asmcode
    | Orelse(e1, e2) -> 
        let labend = newLabel()
        cExpr e1 varEnv funEnv tr pres asmcode
        :: Ins2("cmp", Reg tr, Cst 0) ::
...
and cAccess access varEnv funEnv (tr : reg64) (pres : reg64 list) asmcode: x86 list =
    match access with 
    | AccVar x ->
